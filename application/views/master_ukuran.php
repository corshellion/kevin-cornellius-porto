...
<?php 
	...
	echo "<table border=1 width=300>
			<thead>
			<tr>
				<th><center>Kode Ukuran</th>
				<th><center>Nama Ukuran</th>
				<th><center>Gambar Ukuran</th>
			</tr>
			</thead>
			</tbody>";
	while ($row = mysqli_fetch_array($query_ukuran))
	{
		echo "<tr>
				...
				<td><center>".$row['ukuran']."</td>
				<td><center><img src="."http://localhost/tcreative/assets/admin_data/Ukuran/".$row['gambar_ukuran']." width='150'></td>
			</tr>";
	}
	echo "	</tbody>
		</table><br>";
	...
	mysqli_free_result($query_ukuran);
	
	mysqli_close($conn);
	
	echo "<pre>";
	echo "<font face="."'Century Gothic'"." size="."3".">";
	echo "<form action="."mukuran"." method="."post".">";
	
	echo "<br>Kode Ukuran		: ";
	echo "<input type="."text"." name="."kode_ukuran"." style="."width:170px".">";
		
	echo "<br>Ukuran			: ";
	echo "<input type="."text"." name="."ukuran"." style="."width:170px".">";
	
	echo "<br>Harga			: ";
	echo "<input type="."text"." name="."harga_ukuran"." style="."width:170px".">";
...
//----------INSERT----------
	...
	if(isset($_POST["insert"])){
		...
		$insert_ukuran = "INSERT INTO ukuran VALUES ('$kode_ukuran','$ukuran','$harga_ukuran')";
		
		$query_insert  = mysqli_query($conn, $insert_ukuran);
		if (!$query_insert) {
			die ('SQL Error: ' . mysqli_error($conn));
		}
		header("location:http://localhost/tcreative/Php/mukuran");
	}
	mysqli_close($conn);
//--------------------------
...
//----------UPDATE----------
	...
	if(isset($_POST["update"])){
		...
		$update_ukuran = "UPDATE ukuran SET kode_ukuran='$kode_ukuran',ukuran='$ukuran',harga_ukuran='$harga_ukuran' where kode_ukuran='$kode_ukuran'";
		
		$query_update  = mysqli_query($conn, $update_ukuran);
		if (!$query_update) {
			die ('SQL Error: ' . mysqli_error($conn));
		}
		header("location:http://localhost/tcreative/Php/mukuran");
...
//----------DELETE----------
	...
	if(isset($_POST["delete"])){
		...
		$delete_ukuran="DELETE from ukuran where kode_ukuran='$kode_ukuran'";
		
		$query_delete  = mysqli_query($conn, $delete_ukuran);
		if (!$query_delete) {
			die ('SQL Error: ' . mysqli_error($conn));
		}
		header("location:http://localhost/tcreative/Php/mukuran");

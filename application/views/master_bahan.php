...
<?php
	...
	mysqli_free_result($query_bahan);
	
	mysqli_close($conn);
	
	echo "<pre>";
	echo "<font face="."'Century Gothic'"." size="."3".">";
	echo "<form action="."bahan"." method="."post".">";
	
	echo "<br>Kode Bahan		: ";
	echo "<input type="."text"." name="."kode_bahan"." style="."width:170px".">";
		
	echo "<br>Nama Bahan		: ";
	echo "<input type="."text"." name="."nama_bahan"." style="."width:170px".">";
	
	echo "<br>Harga			: ";
	echo "<input type="."text"." name="."harga_bahan"." style="."width:170px".">";
...
//----------INSERT----------
	...
	if(isset($_POST["insert"])){
		...
		$insert_bahan = "INSERT INTO bahan VALUES ('$kode_bahan','$nama_bahan','$harga_bahan')";
		
		$query_insert  = mysqli_query($conn, $insert_bahan);
		if (!$query_insert) {
			die ('SQL Error: ' . mysqli_error($conn));
		}
		header("location:http://localhost/tcreative/Php/bahan");
	}
	mysqli_close($conn);
//--------------------------
...
//----------UPDATE----------
	...
	if(isset($_POST["update"])){
		...
		$update_bahan = "UPDATE bahan SET kode_bahan='$kode_bahan',nama_bahan='$nama_bahan',harga_bahan='$harga_bahan' where kode_bahan='$kode_bahan'";
		
		$query_update  = mysqli_query($conn, $update_bahan);
		if (!$query_update) {
			die ('SQL Error: ' . mysqli_error($conn));
		}
		header("location:http://localhost/tcreative/Php/bahan");
...
//----------DELETE----------
	...
	if(isset($_POST["delete"])){
		...
		$delete_bahan="DELETE from bahan where kode_bahan='$kode_bahan'";
		
		$query_delete  = mysqli_query($conn, $delete_bahan);
		if (!$query_delete) {
			die ('SQL Error: ' . mysqli_error($conn));
		}
		header("location:http://localhost/tcreative/Php/bahan");

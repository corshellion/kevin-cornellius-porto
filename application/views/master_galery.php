...
<?php 
	...
	echo "<h1>MASTER GALLERY</h1>";
	
	include "conn.php";
	
	$select_gambar = "SELECT * FROM template_galery";

	$query_gambar = mysqli_query($conn, $select_gambar);
	...
	{
		echo "<tr>
				<td><center>".$row['kode_gambar']."</td>
				<td><center><img src="."http://localhost/tcreative/assets/admin_data/galery/".$row['gambar']." width='150'></td>
			</tr>";
	}
	echo "	</tbody>
		</table><br>";
	...
	mysqli_free_result($query_gambar);
	
	mysqli_close($conn);
	
	echo "<pre>";
	echo "<font face="."'Century Gothic'"." size="."3".">";
	echo "<form action="."mgalery"." method="."post".">";
	
	echo "<br>Kode Gambar	: ";
	echo "<input type="."text"." name="."kode_gambar"." style="."width:300px".">";
...
//----------INSERT----------
	...
	if(isset($_POST["insert"])){
		...
		$insert_galery = "INSERT INTO galery VALUES ('$kode_gambar','$gambar')";
		
		$query_insert  = mysqli_query($conn, $insert_galery);
		if (!$query_insert) {
			die ('SQL Error: ' . mysqli_error($conn));
		}
		header("location:http://localhost/tcreative/Php/mgalery");
	}
	mysqli_close($conn);
//--------------------------
...
//----------UPDATE----------
	...
	if(isset($_POST["update"])){
		...
		$update_galery = "UPDATE galery SET kode_gambar='$kode_gambar',gambar='$gambar' where kode_gambar='$kode_gambar'";
		
		$query_update  = mysqli_query($conn, $update_galery);
		if (!$query_update) {
			die ('SQL Error: ' . mysqli_error($conn));
		}
		header("location:http://localhost/tcreative/Php/mgalery");
...
//----------DELETE----------
	...
	if(isset($_POST["delete"])){
		...
		$delete_galery="DELETE from galery where kode_gambar='$kode_gambar'";
		
		$query_delete  = mysqli_query($conn, $delete_galery);
		if (!$query_delete) {
			die ('SQL Error: ' . mysqli_error($conn));
		}
		header("location:http://localhost/tcreative/Php/mgalery");
